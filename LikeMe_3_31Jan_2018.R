...
demand.dump<-cons[, c(1,4:136)]
dd12<-cons[, 137:ncol(cons)]
dd = dd12[,!grepl("^IPython",names(dd12))]
#grep("IPython", colnames(c))
...
#ddss<-data.frame(fread("consolidated_skills1.csv", stringsAsFactors = FALSE))

# 
# cons<-cbind(demand.dump,dd)
                    ...
                    fluidRow(
                      fluidRow(
                        ...
                        box(
                          ...
                          valueBoxOutput("frcst", width = 8),
                          column(4,div(style = "height:150px")),
                          valueBoxOutput("revenue", width = 8)
                        )
...
server <- function(input, output, session) {
  ...
  dd.1<-reactive({
    if(is.null(input$file1))
    {
      ...
      print(ncol(demand.dump))
      dd} else {
        write_csv(as.data.frame(data.append()[,138:ncol(data.append())]),"new_consolidated_skills.csv")
        print("checking done")
  ...
  Append_demand<-function(){
    ...
    infile<-input$file1
    dframe <- read.csv(infile$datapath)
    new.demand <-dframe
    #df$X <- NULL
    ...
    #new.demand <- read.csv("newdemand1234.csv", stringsAsFactors = F)
    ...
    d<-data.frame(t(dd_skills[,1]),row.names = NULL,stringsAsFactors = FALSE)
    ...
    d[1:10,1:3]
    #d<-as.numeric(as.character(dat$x))
    #d <- data.frame(lapply(d, as.character), stringsAsFactors=FALSE)
    #d<-as.character(d)
   # d<-d[-1,]
    #d[, 1:ncol(d)][is.na(d[, 1:ncol(d)])] <- 0
    #d<-replace(is.na(d), 0)
    #d %>% mutate_all(funs(ifelse(is.na(.), 0, .)))
    #d <- gsub('<NA>', 'NA', d)
    #df11 <- tibble(x = c(1, 2, NA), y = c("a", NA, "b"), z = list(1:5, NULL, 10:20))
    #d %>% replace_na(data.frame(x = 0))
    d[is.na(d)]<-0
    library(gtools)
    #library("tidyr")
    ...
    demand.dump12<-as.character(demand.dump)
    demand.dump12<-demand.dump
    #demand.dump11<-replace(is.na(demand.dump11), 0)
    #demand.dump11 %>% replace_na(data.frame(x = 0))
    mutate_all(funs(replace(., is.na(.), 0)))
    demand.dump11<-for (col in 1:ncol(demand.dump11)) set(demand.dump11, which(is.na(x[col])), col, 0)
    demand.dump11<-mutate_at(demand.dump11[1:ncol(demand.dump11)], funs(replace(., is.na(.), 0)))
    demand.dump11 <- demand.dump11 %>% mutate(col = replace(col,is.na(col),0))
  ...
  # print(nrow(demand.dump.1()))
  # print(unique(demand.dump.1()$country))
  # print(unique(demand.dump.1()$Personal.SubArea))
  
  list_customer<- function (customer){
    ########
    dd1<-as.data.frame(dd.1())
    dd1$customer<-demand.dump.1()$Customer
    ##########
    if (customer!=""){
  ...
  newman<-function(input, n, skillbucket, subarea,customer,raio, yea){
    ####below commented lines were to teste the newman fn with appended data#######
    # dd11 <- data.frame(fread("new_consolidated_skills.csv", stringsAsFactors = F,header = TRUE))
    # dd1 <- dd11
    # demand.dump <- data.frame(fread("dump2_created.csv", stringsAsFactors = F))
    # dd1$customer<-demand.dump$Customer
    # demandda<- demand.dump
    # tdd<- t(dd11)
    # tdddataframe<-data.frame(tdd,stringsAsFactors=FALSE)
    # skill<-colnames(dd11)
  ...
  cust.forecast <- function(a,b,c,d,ctry,cc,ps,dc){
    ...
    demand <- subset(demand.dump.1(), dmy(demand.dump.1()$Req.Date)<=n)
    }else demand<-demand.dump.1()
  ...
  combopred <- function(a,b,c,d,ctry,cc,ps,dc){    
    if(wday(maxdate_req)!=7)
    {n<-maxdate_req-wday(maxdate_req)
    demand <- subset(demand.dump.1(), dmy(demand.dump.1()$Approval.Date)<=n)
    }else demand<-demand.dump.1()
  ...
  forecaster_plot <- function(skill.input, ctry,d,cc,ps,dc){
    ...
    #demand <- subset(demand.dump, toupper(demand.dump$country)%in%toupper(ctry))
    if(ctry=="ALL"){
      demand <- demand.dump.1()
    }else{
      demand <- subset(demand.dump.1(), toupper(demand.dump.1()$country)==toupper(ctry))
    }
    ##setwd("D:/HCL/LikeMe/Demand")
  ...
  custskill1 <-function(c, d, e,f,ctry,cc,ps,dc){
    ...
    if(ctry=="ALL"){
      custmaster <- demand.dump.1()
    }else{
      custmaster <- subset(demand.dump.1(), toupper(demand.dump.1()$country)==toupper(ctry))
    }
  ...
  tabs <- function(f,g,h,ctry,d,cc,ps,dc){
    ...
    if(ctry=="ALL"){
      master <- demand.dump.1()
    }else{
      master <- subset(demand.dump.1(), toupper(demand.dump.1()$country)==toupper(ctry))
    }
    if(d!="ALL"){
  ...
  Popular <- function(ctry,customer, n, buky, expe, band, quat){
    ...
    dd[dd>0]<-1
    dd <- dd*demand.dump.1()$InitialDemand
    ...
    A<-1:nrow(demand.dump.1())
    if (customer!=""){
      A<-which(demand.dump.1()$Customer == customer)}
    B<-1:nrow(demand.dump.1())
    if (ctry!="" & ctry!="ALL" ){
      B<-which(toupper(demand.dump.1()$country) == toupper(ctry))}
    C<-1:nrow(demand.dump.1())
    if (buky!="") {
      C<-which(demand.dump.1()$Skill.Bucket==buky)}
    X<-1:nrow(demand.dump.1())
    if (expe!="") {
      X<-which(demand.dump.1()$Experience==expe)}
    Y<-1:nrow(demand.dump.1())
    if (band!="") {
      Y<-which(demand.dump.1()$Band==band)}
    Z<-1:nrow(demand.dump.1())
    if (quat!=""){
      Z<-which(demand.dump.1()$quarter==quat)
    }
    ...
    # 
    demand.dump_USA_Micro<-demand.dump.1()[G,]
  ...
  popularity <- function(ctry,cust, skillbucket){
    cons<-as.data.frame(cons.1()) #by dilip
    #cons <- dem
  ...
  output$varun <- renderUI({
    ...
    switch(input$poploc,
           ...
           "INDIA" = selectInput("dynamic", "Select the Customer",
                                 choices = unique(subset(demand.dump.1(), toupper(demand.dump.1()$country)=="INDIA")$Customer),
                                 selected = "option2"
           ),
           "USA" = selectInput("dynamic", "Select the customer",
                               choices = unique(subset(demand.dump.1(), toupper(demand.dump.1()$country)=="USA")$Customer),
                               selected = "option2"
           )
  ...
  # to filter level2
  output$forecast.cntry <- renderUI({
    selectInput("custloc", "Select the Region for Forecast.",
                choices = unique(demand.dump.1()$country),
                selected = "option3"
    )
  })
  ...
  output$forecast.level <- renderUI({
    ...
    selectInput("forecast.ll", "Select Level 2",
                choices = c("ALL",unique(subset(demand.dump.1(), tolower(demand.dump.1()$country)==tolower(input$custloc))$Joining.Level.2)),
  ...
  output$forecast.cus <- renderUI({
    ...
    selectInput("forecast.cc", "Select The Customer",
                if(input$forecast.ll =="ALL"){
                  choices = c("ALL",unique(subset(demand.dump.1(), tolower(demand.dump.1()$country)==tolower(input$custloc))$Customer))
                }else{choices = c("ALL",unique(subset(demand.dump.1(), tolower(demand.dump.1()$country)==tolower(input$custloc) & tolower(demand.dump.1()$Joining.Level.2)==tolower(input$forecast.ll))$Customer))
                },
                #choices = c("ALL",unique(subset(demand.dump, tolower(demand.dump$country)%in%tolower(input$custloc))$Joining.Level.2)),
                selected = "option3"
    )
  ...
  output$forecast.creg <- renderUI({
    ...
    data1<-subset(demand.dump.1(), tolower(demand.dump.1()$country)==tolower(input$custloc))
    if(input$forecast.ll !="ALL"){
  ...
  output$forecast.cntry1 <- renderUI({
    selectInput("Coun", "Select the Region for Forecast.",
                choices = unique(demand.dump.1()$country),
                selected = "option3"
    )
  })
  ...
  output$skill.varun <- renderUI({
    selectInput("dyna", "Select Skill Bucket",
                choices = unique(subset(subset(demand.dump.1(), toupper(demand.dump.1()$country)==toupper(input$poploc)),
                                        subset(demand.dump.1(), toupper(demand.dump.1()$country)==toupper(input$poploc))$Customer==input$dynamic)$Skill.Bucket),
                selected = "option3"
    )
  })
  ...
  output$pop.cus <- renderUI({
    selectInput("cus", "Select Customer",
                choices = c("", sort(unique(subset(demand.dump.1(),demand.dump.1()$country==input$Coun)$Customer))),
                selected = "option3"
    )
  })  
  ...
  output$pop.quat <- renderUI({
    selectInput("quat", "Select Quarter",
                choices =c("", sort(unique(subset(demand.dump.1(),demand.dump.1()$country==input$Coun & demand.dump.1()$Customer==input$cus)$quarter))),
                selected = "option3"
    )
  })
  ...
  output$pop.buky <- renderUI({
    selectInput("buky", "Select SkillBucket",
                choices =c("", sort(unique(subset(demand.dump.1(),demand.dump.1()$country==input$Coun & demand.dump.1()$Customer==input$cus & demand.dump.1()$quarter==input$quat)$Skill.Bucket))),
                selected = "option3"
    )
  })
  ...
  output$pop.band <- renderUI({
    selectInput("band", "Select Band",
                choices =c("", sort(unique(subset(demand.dump.1(),demand.dump.1()$country==input$Coun & demand.dump.1()$Customer==input$cus & demand.dump.1()$quarter==input$quat & demand.dump.1()$Skill.Bucket==input$buky)$Band))),
                selected = "option3"
    )
  })
  output$pop.expe <- renderUI({
    selectInput("expe", "Select Experiece",
                choices =c("", sort(unique(subset(demand.dump.1(),demand.dump.1()$country==input$Coun & demand.dump.1()$Customer==input$cus & demand.dump.1()$quarter==input$quat & demand.dump.1()$Skill.Bucket==input$buky & demand.dump.1()$Band==input$band)$Experience))),
                selected = "option3"
    )
  })
  #################
